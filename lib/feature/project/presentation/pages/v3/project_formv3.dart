// lib/feature/project/presentation/pages/project_form.dart
...
class ProjectFormDialogv3 extends StatefulWidget {
  final Project? editingProject;
  const ProjectFormDialogv3({super.key, this.editingProject});

  @override
  State<ProjectFormDialogv3> createState() => _ProjectFormDialogState();
}

class _ProjectFormDialogState extends State<ProjectFormDialogv3> {
  ...
  void initState() {
    super.initState();
    _nameCtrl = TextEditingController(text: widget.editingProject?.name ?? '');
    _descCtrl = TextEditingController(text: widget.editingProject?.description ?? '');
    _start = widget.editingProject?.timestamp.startDate;
    _end = widget.editingProject?.timestamp.endDate;
    _status = widget.editingProject?.status ?? Status.pending;
  }
  ...
  void _save() {
    ...
    final ts = TimeStamp(now, now, _start!, _end!);

    if (widget.editingProject == null) {
      final id = DateTime.now().millisecondsSinceEpoch;
      context.read<ProjectBloc>().add(CreateProjectEvent(id, _nameCtrl.text.trim(), _descCtrl.text.trim(), _status, 1, ts));
    } else {
      context.read<ProjectBloc>().add(UpdateProjectEvent(widget.editingProject!.id, _nameCtrl.text.trim(), _descCtrl.text.trim(), _status, widget.editingProject!.createdBy, ts));
    }
  ...
  Widget build(BuildContext context) {
    ...
    final isEdit = widget.editingProject != null;
          ...
          Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(isEdit ? "Chỉnh sửa dự án" : "Thêm dự án mới", style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold, color: cs.onPrimaryContainer)),
                const SizedBox(height: 4),
                Text(isEdit ? "Cập nhật thông tin dự án" : "Tạo dự án mới", style: Theme.of(context).textTheme.bodySmall?.copyWith(color: cs.onPrimaryContainer.withOpacity(0.7))),
              ])),
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  TextFormField(controller: _nameCtrl, decoration: _dec(cs, "Tên dự án", "Nhập tên dự án"), validator: (v) => v?.isEmpty ?? true ? "Vui lòng nhập tên" : null),
                  const SizedBox(height: 20),
                  TextFormField(controller: _descCtrl, decoration: _dec(cs, "Mô tả", "Nhập mô tả chi tiết"), maxLines: 3),
                  const SizedBox(height: 20),
                  Row(children: [
